...
function renderVidgen2Page() {
  ...
  const isKling = state.vidgen2.selectedModel === 'kling-v3';
  const models = [
    ...
    { id: 'kling-v3', name: 'Kling V3', desc: 'Video 5-15 detik, 1080P, Audio', badge: 'V3', icon: '🎥' }
  ];
  ...
  if (!window._vidgen2DelegationAttached) {
    ...
    document.addEventListener('click', function(e) {
      ...
      if (modelCard && state.currentPage === 'vidgen2') {
        ...
        const validDurations = newModel === 'grok-video-3-10s' ? [10] : newModel === 'vidu-q3-turbo' ? [4, 8, 10] : newModel === 'kling-v3' ? [5, 10, 15] : [5, 8];
        if (!validDurations.includes(state.vidgen2.duration)) {
          state.vidgen2.duration = validDurations[validDurations.length - 1];
        }
        const validResolutions = (newModel === 'veo-3.1-fast' || newModel === 'veo-3.1') ? ['4K'] : (newModel === 'vidu-q3-turbo' || newModel === 'kling-v3') ? ['720P', '1080P'] : ['720P'];
