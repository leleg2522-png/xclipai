...
async function pollFreepikImageTask(taskId, apiKey, endpoint) {
  try {
    const resp = await makeFreepikRequest(
      'GET',
...
async function generateImageWithFreepikNanoBanana(prompt, aspectRatio, refImageUrls, userId, options = {}) {
  ...
  for (let keyAttempt = 0; keyAttempt < maxKeyRetries; keyAttempt++) {
    ...
    for (let createAttempt = 0; createAttempt < maxCreateRetries && !createSucceeded; createAttempt++) {
      try {
        createResp = await makeFreepikRequest(
          'POST',
...
async function resumePendingTaskPolling() {
  try {
    const tables = [
      ...
      { table: 'ximage2_history', apiType: 'freepik-image', urlCol: 'image_url', keyCol: null },
        ...
        for (const row of result.rows) {
          ...
          if (apiKey) {
            ...
            const bgPollOpts = {
              ...
              endpoint: (resolvedType === 'freepik-image' && XIMAGE2_MODELS[row.model]) ? XIMAGE2_MODELS[row.model].freepikEndpoint : undefined
            };
...
app.post('/api/videogen/proxy', async (req, res) => {
  try {
    ...
    const rawAR = aspectRatio === 'square_1_1' ? '1:1'
      : aspectRatio === 'social_story_9_16' ? '9:16'
      : aspectRatio === 'widescreen_16_9' ? '16:9'
      : (['1:1', '9:16', '16:9'].includes(aspectRatio) ? aspectRatio : '16:9');
    const mappedAR = rawAR === '1:1' ? 'square_1_1'
      : rawAR === '9:16' ? 'social_story_9_16' : 'widescreen_16_9';
...
app.post('/api/generate-video', async (req, res) => {
  try {
    ...
    const rawAR3 = aspectRatio === 'widescreen_16_9' ? '16:9'
      : aspectRatio === 'social_story_9_16' ? '9:16'
      : aspectRatio === 'square_1_1' ? '1:1'
      : (['1:1', '9:16', '16:9'].includes(aspectRatio) ? aspectRatio : '16:9');
    const mappedAR3 = rawAR3 === '1:1' ? 'square_1_1'
      : rawAR3 === '9:16' ? 'social_story_9_16' : 'widescreen_16_9';
  ...
  try {
    ...
    (async () => {
      try {
        ...
        console.error(`[VIDGEN3] Background error: ${taskId}: ${errStr}`);
...
// ============ X IMAGE2 (Freepik Image Generation) ============

const XIMAGE2_MODELS = {
  'nano-banana-pro': {
    name: 'Nano Banana Pro', provider: 'Freepik (Google)', supportsI2I: true,
  ...
  'nano-banana-pro-flash': {
    name: 'Nano Banana Pro Flash', provider: 'Freepik (Google)', supportsI2I: true,
    sizes: ['1:1', '2:3', '3:2', '4:3', '3:4', '5:4', '4:5', '16:9', '9:16', '21:9'],
    maxN: 1, maxRefs: 3,
    resolutions: ['1K', '2K', '4K'], defaultResolution: '2K',
    freepikEndpoint: '/v1/ai/text-to-image/nano-banana-pro-flash',
    desc: 'Google Nano Banana Pro Flash via Freepik - fast generation'
  },
  'seedream-v5-lite': {
    name: 'Seedream V5 Lite', provider: 'Freepik (ByteDance)', supportsI2I: true,
    sizes: ['1:1', '2:3', '3:2', '4:3', '3:4', '5:4', '4:5', '16:9', '9:16', '21:9'],
    maxN: 1, maxRefs: 3,
    resolutions: ['1K', '2K', '4K'], defaultResolution: '2K',
    freepikEndpoint: '/v1/ai/text-to-image/seedream-v5-lite',
    desc: 'ByteDance Seedream V5 Lite via Freepik - balanced quality'
  },
  'flux': {
...
async function getXImage2PoolKey(xclipApiKey) {
  const keyInfo = await validateXclipApiKey(xclipApiKey);
  if (!keyInfo) {
    return { error: 'Xclip API key tidak valid' };
  }

  const poolKeys = await getPoolKeysForFeature(keyInfo.user_id, 'ximage2', 3);
  if (poolKeys.length > 0) {
    const selectedKey = poolKeys[0];
    return {
      apiKey: selectedKey.key,
      keyName: selectedKey.name,
      poolId: selectedKey.poolId,
      isPool: true,
      userId: keyInfo.user_id,
      keyInfoId: keyInfo.id
    };
  ...
  return { error: 'Tidak ada API key X Image2 yang tersedia di pool. Hubungi admin.' };
}
...
app.post('/api/ximage2/generate', async (req, res) => {
  ...
  try {
    ...
    poolKeyResult = await getXImage2PoolKey(xclipApiKey);
    if (poolKeyResult.error) {
    ...
    console.log(`[XIMAGE2] Generating with Freepik model: ${model}, size: ${size || 'default'}, i2i: ${isI2I}`);
    ...
    if (isI2I) {
      ...
      for (let i = 0; i < Math.min(images.length, modelConfig.maxRefs || 3); i++) {
        ...
        if (img && img.startsWith('data:')) {
          ...
          console.log(`[XIMAGE2] Image ${i + 1} uploaded: ${imgUrl}`);
        }
        if (imgUrl) {
      ...
      if (isSeedream && refUrls.length > 0) {
        // Seedream i2i uses the image-to-image endpoint with array of URL strings
    ...
    if (respData.generated && Array.isArray(respData.generated) && respData.generated.length > 0) {
      ...
      if (!directImageUrl && g.base64) {
        directImageUrl = `data:image/png;base64,${g.base64}`;
      }
    }
    ...
    if (directImageUrl) {
      ...
      `, [poolKeyResult.userId, 'direct-' + Date.now(), model, prompt, isI2I ? 'image-to-image' : 'text-to-image', size || '1:1', directImageUrl]);

      setUserCooldown(poolKeyResult.userId, 'ximage2');
      if (poolKeyResult.poolId) await recordKeyUsage(poolKeyResult.poolId);
      await pool.query(
        'UPDATE xclip_api_keys SET requests_count = requests_count + 1, last_used_at = CURRENT_TIMESTAMP WHERE id = $1',
        [poolKeyResult.keyInfoId]
      );
    ...
    if (!taskId) {
      ...
      return res.status(500).json({ error: 'Tidak mendapat task ID dari Freepik' });
    }

    await pool.query(`
      INSERT INTO ximage2_history (user_id, task_id, model, prompt, mode, size, reference_image, status)
      VALUES ($1, $2, $3, $4, $5, $6, $7, 'processing')
    `, [poolKeyResult.userId, taskId, model, prompt, isI2I ? 'image-to-image' : 'text-to-image', size || '1:1', isI2I ? 'ref_uploaded' : null]);
    ...
    startServerBgPoll(taskId, 'freepik-image', poolKeyResult.apiKey, {
      dbTable: 'ximage2_history',
      urlColumn: 'image_url',
      model: model,
      userId: poolKeyResult.userId,
      endpoint: freepikEndpoint
    });
    ...
    const errData = error.response?.data;
    const rawMsg = errData?.detail || errData?.message || errData?.error || error.message;
    const httpStatus = error.response?.status;
    console.error('[XIMAGE2] Generate error:', JSON.stringify(errData || error.message));
    console.error('[XIMAGE2] Status:', httpStatus, 'Model:', req.body?.model);

    if (httpStatus === 402 || httpStatus === 429) {
      if (poolKeyResult && poolKeyResult.poolId) {
        const replacement = await handlePoolKeyExhausted(poolKeyResult.poolId, poolKeyResult.userId, 'ximage2', `HTTP ${httpStatus}`);
        if (replacement) {
          return res.status(429).json({ error: 'API key kena rate limit, silakan coba lagi.' });
        }
      }
    }
    ...
    if (userMsg.includes('timeout') || userMsg.includes('ETIMEDOUT')) {
      userMsg = 'Request timeout. Freepik terlalu lama merespon, coba lagi.';
    }

    res.status(httpStatus || 500).json({ error: userMsg });
  }
});
...
app.get('/api/ximage2/status/:taskId', async (req, res) => {
  try {
    ...
    const poolKeyResult = await getXImage2PoolKey(xclipApiKey);
    if (poolKeyResult.error) {
      return res.status(400).json({ error: poolKeyResult.error });
    }
    ...
    if (localTask.rows.length > 0 && localTask.rows[0].status === 'failed') {
      return res.json({ status: 'failed', error: 'Image generation failed' });
    }
    ...
    // Seedream i2i tasks live under the image-to-image endpoint
    if (model === 'seedream-v5-lite' && taskMode === 'image-to-image') {
      pollEndpoint = '/v1/ai/image-to-image/seedream-v5-lite';
    }
    ...
    if (pollResult.status === 'completed' && pollResult.url) {
      await pool.query(`
        UPDATE ximage2_history SET status = 'completed', image_url = $1, completed_at = NOW() WHERE task_id = $2
      `, [pollResult.url, taskId]);
      return res.json({ status: 'completed', imageUrl: pollResult.url });
    }

    if (pollResult.status === 'failed') {
      await pool.query(`
        UPDATE ximage2_history SET status = 'failed', completed_at = NOW() WHERE task_id = $1
      `, [taskId]);
      return res.json({ status: 'failed', error: pollResult.error || 'Generation failed' });
    }

    res.json({ status: 'processing', progress: 0, message: 'Image sedang diproses...' });
...
async function generateVideoWithFreepik(imageUrl, prompt, aspectRatio, model, userId, feature, videoDuration, referenceImages, language) {
  ...
  function simplifyPromptForVideo(rawPrompt) {
    ...
    if (dialogMatch) {
      if (isNonEnglish) {
        dialogPart = `, character speaking naturally in ${langLabel}, natural mouth movement, talking to camera`;
      } else {
        ...
        dialogPart = `, character speaking in English saying "${dialogText}"`;
      }
    }
  ...
  if (config.api === 'kling26') {
    requestBody = {
      ...
      aspect_ratio: mappedAR, negative_prompt: adsNegPrompt,
  ...
  for (let attempt = 0; attempt < userKeys.length + 6; attempt++) {
    ...
    try {
      console.log(`[KEY-POOL] Trying key ${keyRecord.id} (${keyRecord.api_key.substring(0, 8)}...) for ${feature} via Decodo`);
      const response = await makeFreepikRequest(
        'POST',
      ...
      const isInvalidKey = isInvalidFreepikKey(err);
      if (httpStatus === 402 || httpStatus === 429 || httpStatus === 403 || httpStatus === 401 || isInvalidKey) {
        const reason = isInvalidKey ? `INVALID KEY (HTTP ${httpStatus || 'n/a'})` : `HTTP ${httpStatus}`;
        console.warn(`[KEY-POOL] Killing dead key ${keyRecord.id} for ${feature}: ${reason}`);
        triedKeys.push(keyRecord.api_key);
        await markKeyExhausted(keyRecord.id, `${reason}: ${err.response?.data?.message || err.message}`);
...
async function initDatabase() {
  try {
    ...
    await pool.query(`
      CREATE TABLE IF NOT EXISTS ads_studio_projects (
        id SERIAL PRIMARY KEY,
        user_id INTEGER REFERENCES users(id) ON DELETE CASCADE,
        project_id VARCHAR(255) UNIQUE,
        title VARCHAR(500),
    ...
    await pool.query(`
      CREATE TABLE IF NOT EXISTS ads_studio_scenes (
        id SERIAL PRIMARY KEY,
        project_id VARCHAR(255) REFERENCES ads_studio_projects(project_id) ON DELETE CASCADE,
        scene_index INTEGER NOT NULL,
        narration TEXT,
        text_overlay TEXT,
        visual_prompt TEXT,
...
app.get('/api/ads-studio/projects', async (req, res) => {
  if (!req.session.userId) return res.status(401).json({ error: 'Login required' });
  try {
    const result = await pool.query(
      `SELECT * FROM ads_studio_projects WHERE user_id = $1 ORDER BY created_at DESC LIMIT 50`,
      [req.session.userId]
    );
    res.json({ projects: result.rows });
  } catch (error) {
    console.error('[ADS-STUDIO] List error:', error.message);
    res.status(500).json({ error: 'Gagal memuat project' });
  }
});

app.get('/api/ads-studio/projects/:projectId', async (req, res) => {
  if (!req.session.userId) return res.status(401).json({ error: 'Login required' });
  try {
    const projResult = await pool.query(
    ...
    if (projResult.rows.length === 0) return res.status(404).json({ error: 'Project not found' });
    const scenes = await pool.query(
      `SELECT * FROM ads_studio_scenes WHERE project_id = $1 ORDER BY scene_index ASC`,
      [req.params.projectId]
    );
    res.json({ project: projResult.rows[0], scenes: scenes.rows });
  } catch (error) {
    console.error('[ADS-STUDIO] Detail error:', error.message);
    res.status(500).json({ error: 'Gagal memuat project' });
  }
});

app.delete('/api/ads-studio/projects/:projectId', async (req, res) => {
  if (!req.session.userId) return res.status(401).json({ error: 'Login required' });
  try {
    const result = await pool.query(
      `DELETE FROM ads_studio_projects WHERE project_id = $1 AND user_id = $2 RETURNING id`,
      [req.params.projectId, req.session.userId]
    );
    if (result.rowCount === 0) return res.status(404).json({ error: 'Project not found' });
    res.json({ success: true });
  } catch (error) {
    console.error('[ADS-STUDIO] Delete error:', error.message);
    res.status(500).json({ error: 'Gagal menghapus project' });
  }
});
...
app.post('/api/ads-studio/projects', upload.fields([
  ...
  try {
    ...
    if (req.files && req.files['characterImage'] && req.files['characterImage'][0]) {
      characterImageUrl = `${baseUrl}/uploads/${req.files['characterImage'][0].filename}`;
    }
    ...
    console.error('[ADS-STUDIO] Create error:', error.message);
    res.status(500).json({ error: 'Gagal membuat project' });
  }
});

app.post('/api/ads-studio/projects/:projectId/generate-script', async (req, res) => {
  if (!req.session.userId) return res.status(401).json({ error: 'Login required' });
  const { projectId } = req.params;
  try {
    const projResult = await pool.query(
    ...
    await pool.query(
      `UPDATE ads_studio_projects SET status = 'generating_script', updated_at = NOW() WHERE project_id = $1`,
      [projectId]
    );
    sendSSEToUser(req.session.userId, { type: 'ads_studio_update', projectId, status: 'generating_script' });

    const adTypeDesc = project.ad_type === 'hard_selling' ? 'Hard Selling (langsung jualan, CTA kuat, urgensi tinggi, fokus benefit dan harga)' : 'Soft Selling (storytelling, lifestyle, edukasi, produk muncul natural tanpa jualan langsung)';
    const formatDesc = project.format === 'shorts' ? 'YouTube Shorts / TikTok / Reels (vertical 9:16, 30-60 detik total)' : 'YouTube / landscape ad (16:9, 1-2 menit total)';
    const langName = project.language === 'en' ? 'English' : 'Bahasa Indonesia';
...
CONTINUITY: Same character, same product, same setting, same clothing. Each scene flows logically to the next like ONE continuous story.`;
    ...
    if (!apimodelsKey) {
      await pool.query(
        `UPDATE ads_studio_projects SET status = 'script_failed', error_message = 'ApiModels API key not configured', updated_at = NOW() WHERE project_id = $1`,
        [projectId]
      );
      sendSSEToUser(req.session.userId, { type: 'ads_studio_update', projectId, status: 'script_failed' });
      return res.status(500).json({ error: 'ApiModels API key not configured' });
    }
    ...
    res.json({ success: true, message: 'Generating script...' });
    ...
    (async () => {
      ...
      for (const model of chatModels) {
        try {
          console.log(`[ADS-STUDIO] Trying chat model: ${model}`);
          ...
          const raw = chatResponse.data;
          aiContent = raw?.content?.[0]?.text || raw?.choices?.[0]?.message?.content || (typeof raw?.content === 'string' ? raw.content : null);
          if (aiContent) {
            console.log(`[ADS-STUDIO] Got response from ${model}`);
            break;
          }
        } catch (chatErr) {
          lastChatErr = chatErr.message;
          console.error(`[ADS-STUDIO] Model ${model} failed:`, chatErr.message);
        }
      }

      if (!aiContent) {
        await pool.query(
          `UPDATE ads_studio_projects SET status = 'script_failed', error_message = $2, updated_at = NOW() WHERE project_id = $1`,
          [projectId, 'All AI models failed: ' + (lastChatErr || 'Unknown')]
        );
        sendSSEToUser(project.user_id, { type: 'ads_studio_update', projectId, status: 'script_failed' });
        return;
      }
      ...
      try {
        ...
        if (!parsed.scenes || !Array.isArray(parsed.scenes) || parsed.scenes.length === 0) {
          await pool.query(
            `UPDATE ads_studio_projects SET status = 'script_failed', error_message = 'AI returned no scenes', updated_at = NOW() WHERE project_id = $1`,
            [projectId]
          );
          sendSSEToUser(project.user_id, { type: 'ads_studio_update', projectId, status: 'script_failed' });
          return;
        ...
        await pool.query(
          `UPDATE ads_studio_projects SET title = $2, script = $3, status = 'script_ready', updated_at = NOW() WHERE project_id = $1`,
          [projectId, parsed.title || project.product_name, JSON.stringify(parsed)]
        );
        await pool.query(`DELETE FROM ads_studio_scenes WHERE project_id = $1`, [projectId]);
        ...
        sendSSEToUser(project.user_id, { type: 'ads_studio_update', projectId, status: 'script_ready' });
        console.log(`[ADS-STUDIO] Script ready for ${projectId}: ${parsed.scenes.length} scenes`);
      } catch (parseErr) {
        console.error(`[ADS-STUDIO] Parse error:`, parseErr.message);
        await pool.query(
          `UPDATE ads_studio_projects SET status = 'script_failed', error_message = $2, updated_at = NOW() WHERE project_id = $1`,
          [projectId, 'Failed to parse AI response: ' + parseErr.message]
        );
        sendSSEToUser(project.user_id, { type: 'ads_studio_update', projectId, status: 'script_failed' });
      }
    })();
    ...
    console.error('[ADS-STUDIO] Generate script error:', error.message);
    res.status(500).json({ error: 'Gagal generate script' });
  }
});

app.put('/api/ads-studio/projects/:projectId/scenes/:sceneIndex', async (req, res) => {
  if (!req.session.userId) return res.status(401).json({ error: 'Login required' });
  const { projectId, sceneIndex } = req.params;
  try {
    const projCheck = await pool.query(
      `SELECT status FROM ads_studio_projects WHERE project_id = $1 AND user_id = $2`,
      [projectId, req.session.userId]
    );
    if (projCheck.rows.length === 0) return res.status(404).json({ error: 'Project not found' });
    ...
    await pool.query(
      `UPDATE ads_studio_scenes SET ${updateFields.join(', ')} WHERE project_id = $1 AND scene_index = $2`,
      updateValues
    );
    res.json({ success: true });
  } catch (error) {
    console.error('[ADS-STUDIO] Update scene error:', error.message);
    res.status(500).json({ error: 'Gagal update scene' });
  }
});

app.post('/api/ads-studio/projects/:projectId/start', async (req, res) => {
  if (!req.session.userId) return res.status(401).json({ error: 'Login required' });
  const { projectId } = req.params;
  try {
    const projResult = await pool.query(
    ...
    const lockResult = await pool.query(
      `UPDATE ads_studio_projects SET status = 'producing', error_message = NULL, updated_at = NOW() WHERE project_id = $1 AND status IN ('script_ready', 'production_failed') RETURNING project_id`,
      [projectId]
    );
    if (lockResult.rowCount === 0) {
      return res.status(409).json({ error: 'Production sudah berjalan' });
    ...
    (async () => {
      try {
        ...
        let productRefUrl = project.product_image_url || null;
        let prevSceneImageUrl = null;
        let scene1ImageUrl = null;
        ...
        for (const scene of scenes.rows) {
          ...
          if (!scene.image_url) {
            await pool.query(
              `UPDATE ads_studio_scenes SET status = 'generating_image', updated_at = NOW() WHERE project_id = $1 AND scene_index = $2`,
              [projectId, scene.scene_index]
            );
            sendSSEToUser(project.user_id, { type: 'ads_studio_scene_update', projectId, sceneIndex: scene.scene_index, status: 'generating_image' });

            try {
              const refImages = [];
              let refPrompt;

              if (characterRefUrl) refImages.push(characterRefUrl);
              if (productRefUrl) refImages.push(productRefUrl);
              ...
              function toStaticImagePrompt(vp) {
                if (!vp) return '';
                let s = vp;
                s = s.replace(/(?:character\s+)?(?:speaks?|talks?|says?|saying|speaking)\s+(?:saying\s+)?["'][^"']*["']/gi, '');
                s = s.replace(/\b(camera\s+(?:pushes?|pulls?|tracks?|tilts?|orbits?|racks?|drifts?|pans?|zooms?)[^,]*)/gi, '');
                s = s.replace(/\b(slow[- ]motion|real[- ]time|gradually|suddenly)\b/gi, '');
                s = s.replace(/\b(rustles?|sways?|drifts?|flickers?|ripples?|flows?|floats?|shifts?)\b/gi, '');
                s = s.replace(/,\s*,/g, ',').replace(/\s{2,}/g, ' ').trim();
                return s;
              }

              const staticPrompt = toStaticImagePrompt(scene.visual_prompt);
              ...
              if (refImages.length > 0) {
                ...
                refPrompt = `REFERENCE IMAGES:\n${refDesc.join('\n')}\n\nSCENE TO GENERATE (still image): ${staticPrompt}\n${charDesc ? `\nCHARACTER (MUST be identical in every scene): ${charDesc}` : ''}${settingDesc ? `\nSETTING (SAME location in every scene): ${settingDesc}` : ''}${prodVisual ? `\nPRODUCT: ${prodVisual}` : ''}\n\nCRITICAL — THIS IS THE MOST IMPORTANT RULE:\nThe character in this image MUST be the EXACT SAME PERSON as in the reference images. Copy the face pixel-by-pixel. Same face shape, eyes, nose, mouth, skin tone, hair color, hair length, hairstyle, clothing color, clothing style. If you change ANYTHING about the character's appearance, the image is WRONG.\n\nADDITIONAL RULES:\n- SAME background/room as reference images — same walls, furniture, objects, floor\n- SAME lighting direction and color temperature\n- Product must look identical — same shape, colors, size, label\n- NO text, titles, captions, watermarks, logos anywhere\n- NO reflective/glossy floors`;
              } else {
              ...
              await pool.query(
                `UPDATE ads_studio_scenes SET image_url = $3, updated_at = NOW() WHERE project_id = $1 AND scene_index = $2`,
                [projectId, scene.scene_index, imageUrl]
              );
              scene.image_url = imageUrl;
              sendSSEToUser(project.user_id, { type: 'ads_studio_scene_update', projectId, sceneIndex: scene.scene_index, status: 'image_ready', imageUrl });
              console.log(`[ADS-STUDIO] Scene ${scene.scene_index} image ready: ${imageUrl}`);
            } catch (imgErr) {
              console.error(`[ADS-STUDIO] Scene ${scene.scene_index} image failed:`, imgErr.message);
              await pool.query(
                `UPDATE ads_studio_scenes SET status = 'failed', error_message = $3, updated_at = NOW() WHERE project_id = $1 AND scene_index = $2`,
                [projectId, scene.scene_index, 'Image: ' + imgErr.message]
              );
              sendSSEToUser(project.user_id, { type: 'ads_studio_scene_update', projectId, sceneIndex: scene.scene_index, status: 'failed', error: imgErr.message });
              continue;
            }
          }
        }
        ...
        const completedScenes = await pool.query(
          `SELECT COUNT(*) as total, COUNT(*) FILTER (WHERE status = 'completed') as completed, COUNT(*) FILTER (WHERE status = 'failed') as failed
           FROM ads_studio_scenes WHERE project_id = $1`,
          [projectId]
        );
        const stats = completedScenes.rows[0];
        const allDone = parseInt(stats.completed) + parseInt(stats.failed) === parseInt(stats.total);
        ...
        if (allDone) {
          if (parseInt(stats.failed) === parseInt(stats.total)) {
            await pool.query(
              `UPDATE ads_studio_projects SET status = 'production_failed', error_message = 'Semua scene gagal', updated_at = NOW() WHERE project_id = $1`,
              [projectId]
            );
            sendSSEToUser(project.user_id, { type: 'ads_studio_update', projectId, status: 'production_failed' });
          } else if (parseInt(stats.completed) > 0) {
            const completedScenesData = await pool.query(
              `SELECT * FROM ads_studio_scenes WHERE project_id = $1 AND status = 'completed' AND video_url IS NOT NULL ORDER BY scene_index ASC`,
              [projectId]
            );
            ...
            if (completedScenesData.rows.length > 1) {
              try {
                ...
                await pool.query(
                  `UPDATE ads_studio_projects SET status = 'completed', final_video_url = $2, updated_at = NOW(), completed_at = NOW() WHERE project_id = $1`,
                  [projectId, mergedUrl]
                );
                ...
                console.error(`[ADS-STUDIO] Merge failed:`, mergeErr.message);
                await pool.query(
                  `UPDATE ads_studio_projects SET status = 'completed', error_message = $2, updated_at = NOW(), completed_at = NOW() WHERE project_id = $1`,
                  [projectId, 'Merge failed: ' + mergeErr.message]
                );
                sendSSEToUser(project.user_id, { type: 'ads_studio_update', projectId, status: 'completed' });
              }
            } else {
              await pool.query(
                `UPDATE ads_studio_projects SET status = 'completed', updated_at = NOW(), completed_at = NOW() WHERE project_id = $1`,
                [projectId]
              );
              sendSSEToUser(project.user_id, { type: 'ads_studio_update', projectId, status: 'completed' });
            }
        ...
        console.error(`[ADS-STUDIO] Pipeline error for ${projectId}:`, pipelineErr.message);
        await pool.query(
          `UPDATE ads_studio_projects SET status = 'production_failed', error_message = $2, updated_at = NOW() WHERE project_id = $1`,
          [projectId, pipelineErr.message]
        ).catch(() => {});
        sendSSEToUser(project.user_id, { type: 'ads_studio_update', projectId, status: 'production_failed' });
      }
    ...
    console.error('[ADS-STUDIO] Start production error:', error.message);
    res.status(500).json({ error: 'Gagal memulai produksi' });
  }
});
...
app.post('/api/ads-studio/projects/:projectId/merge', async (req, res) => {
  ...
  try {
    ...
    if (completedScenes.rows.length < 2) return res.status(400).json({ error: 'Minimal 2 scene yang selesai untuk merge' });

    await pool.query(`UPDATE ads_studio_projects SET status = 'merging', updated_at = NOW() WHERE project_id = $1`, [projectId]);
    sendSSEToUser(req.session.userId, { type: 'ads_studio_update', projectId, status: 'merging' });

    res.json({ success: true, message: 'Merging...' });
    ...
    (async () => {
      try {
        ...
        console.error(`[ADS-STUDIO] Merge failed:`, mergeErr.message);
        await pool.query(
          `UPDATE ads_studio_projects SET status = 'completed', error_message = $2, updated_at = NOW() WHERE project_id = $1`,
          [projectId, 'Merge failed: ' + mergeErr.message]
        );
        sendSSEToUser(project.user_id, { type: 'ads_studio_update', projectId, status: 'completed' });
      }
